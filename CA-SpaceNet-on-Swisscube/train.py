...
def train(cfg, epoch, max_epoch, loader, model, model_c, optimizer, scheduler, device, logger=None):
    model.train()
    model_c.eval()  # The counterfactual path requires no training
    ...
    for idx, (images, masked, targets, _) in pbar:
        ...
        # Extract the feature of CF images
        features_c = model_c(images_c)

        # Forware
        loss_dict = model(images, features_c, targets=targets)
    ...
    if os.path.exists(cfg['RUNTIME']['WEIGHT_FILE']):  # --weight_file
        try:
            ...
            if 'model' in chkpt:
                ...
                model_c.load_state_dict(chkpt, strict=False)  # Ignore um-match layers
...
if __name__ == '__main__':
    ...
    for i, p in enumerate(model_c.parameters()):  # Freezing all params in Counterfactual_path
        p.requires_grad = False
