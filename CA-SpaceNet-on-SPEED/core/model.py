...
from core.utils import load_bbox_3d
...
class TargetCoder(object):
    ...
    def decode(self, preds, anchors):
        ...
        ptx = preds[:, :11] * anchors_w.view(-1, 1) + anchors_cx.view(-1, 1)
        pty = preds[:, 11:] * anchors_h.view(-1, 1) + anchors_cy.view(-1, 1)
...
class PoseHead(nn.Module):
    def __init__(self, in_channel, n_class, n_conv, prior):
        ...
        self.pose_pred = nn.Conv2d(
            in_channel, num_anchors * num_classes * 22, kernel_size=3, stride=1,
            padding=1
        )
    ...
    def forward(self, x):
        ...
        for l, feature in enumerate(x):
            ...
            pose_reg.append(pose_pred)  # torch.Size([8, 22, 64, 64]) ...
...
class Counterfactual_path(nn.Module):
    ...
    def forward(self, c_images):
        ...
        c_features = self.fpn(c_features)  # 反事实通路，获取此通路特征
...
class PoseModule_Casual(nn.Module):
    ...
    def forward(self, images, features_c, targets):
        ...
        features = self.fpn(img_features)  # 正常通路，将主体信息与背景信息全部纳入
        features_sub = self.fpn_sub(img_features)  # 模拟通路，用于模拟外部的纯背景信息
        ...
        for i in range(5):
            features[i] = features[i] - 0.1 * features_sub[i]  # 特征相减，去除背景影响
        # features = [features[-1]] # disable FPN and pick up only the deepest features
        ...
        if self.training:
            loss_cls, loss_reg = self.loss_evaluator(pred_cls, pred_reg, targets, anchors)
            loss_sim = 0
            for i in range(5):  # 度量features_c和features_sub之间的相似度损失
                loss_sim += nn.SmoothL1Loss(reduction='mean')(features_sub[i], features_c[i])
                # 为什么这里的loss如此的大？0.6左右，按道理可以直接学习到FPN的映射
            loss_sim = loss_sim * 0.25
