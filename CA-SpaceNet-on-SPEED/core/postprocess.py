...
from core.loss import permute_and_flatten
...
class PostProcessor(nn.Module):
    ...
    def forward_for_single_feature_map(self, sCls, sReg, sAnchors):
        ...
        sReg = permute_and_flatten(sReg, N, A, C*22, H, W)
        sReg = sReg.reshape(N, -1, C*22)
        ...
        for per_sCls, per_sReg, per_pre_ransac_top_n, per_candidate_inds, per_anchors \
            ...
            detections = self.box_coder.decode(
                per_sReg.view(-1, C, 22)[per_box_loc, per_class],
    ...
    def select_over_all_levels(self, pred_inter_list, targets):
        ...
        for i in range(num_images):
            xy2d_np = None
            result ,xy2d_np = self.pose_infer_ml(pred_inter_list[i], targets[i])
            results.append(result)
            xy2d_nps.append(xy2d_np)
        return results, xy2d_nps[0]
